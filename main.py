...
from typing import Any, Callable, Dict, List, Optional  # STEP 1: Aggiunto Callable
from schemas import (
...
# STEP 2: Configurazione job asincroni
ANALYSIS_JOB_WORKERS = int(os.getenv("ANALYSIS_JOB_WORKERS", "2"))

# F13: Rate limit state
RATE_LIMIT_STATE = {}
RATE_LIMIT_WINDOW_SECONDS = 3600

# STEP 3: Registro job in memoria
JOB_STATUSES = {"queued", "processing", "done", "error", "cancelled"}

jobs_lock = Lock()
jobs_store: Dict[str, Dict[str, Any]] = {}
jobs_executor = ThreadPoolExecutor(max_workers=ANALYSIS_JOB_WORKERS)
...
def compute_geojson_hash(geojson: Dict[str, Any]) -> str:
    """
    Calcola un hash stabile della geometria GeoJSON.
    Usato dal lifecycle dei job per audit, sicurezza e futura cache.
    """
    payload = json.dumps(
        geojson,
        sort_keys=True,
...
def get_job_record_or_404(job_id: str, auth_id: str) -> Dict[str, Any]:
    with jobs_lock:
...
def job_to_status_response(job: Dict[str, Any]) -> Dict[str, Any]:
    payload = {
        "job_id": job["job_id"],
        "status": job["status"],
...
def emit_analysis_progress(
    ...
    try:
        ...
        print(f"[WARN] progress_callback_failed step={step}")
        print(str(exc))


# ================================================================
# STEP 6: VALIDAZIONE CONTRATTO DATI
# ================================================================
...
# ================================================================
# STEP 5: RUN ANALYSIS CORE (con progress callback)
# ================================================================
...
def run_analysis_core(
    ...
    """
    Nucleo dell'analisi satellitare.
    Può essere chiamato da /analyze (sincrono) o da run_analysis_job (asincrono).
    """
    request_id = request_id_override or str(uuid.uuid4())[:8]
    stage_time = time.time()

    # STEP 4: Progress checkpoint - Validazione area
    emit_analysis_progress(
        progress_callback,
        "Validazione area di studio",
        8.0,
    )
    ...
    # STEP 5: Progress checkpoint - Area validata
    emit_analysis_progress(
        progress_callback,
        "Area validata",
        12.0,
    ...
    scale = 10
    cloudThreshold = payload.get("cloud_threshold", 0.40)
    validPixelThreshold = payload.get("valid_pixel_threshold", 10.0)
    mahalAlpha = payload.get("mahal_alpha", 0.975)
    minClusterPixels = max(1, int(payload.get("min_cluster_pixels", 4)))
    applySpatialSmoothing = payload.get("apply_spatial_smoothing", True)
    covShrinkage = min(max(payload.get("covariance_shrinkage", 0.15), 0.0), 0.9)
    robustCovIterations = max(0, int(payload.get("robust_covariance_iterations", 2)))
    ...
    # STEP 6: Progress checkpoint - Immagini Sentinel-2 filtrate
    emit_analysis_progress(
        progress_callback,
        "Immagini Sentinel-2 filtrate",
        25.0,
    )
    ...
    # STEP 7: Progress checkpoint - Serie temporale elaborata
    emit_analysis_progress(
        progress_callback,
        "Serie temporale elaborata",
        38.0,
    )
    ...
    # STEP 8: Progress checkpoint - Soglia statistica calcolata
    emit_analysis_progress(
        progress_callback,
        "Soglia statistica calcolata",
        55.0,
    )
    ...
    # STEP 9: Progress checkpoint - Controlli di significatività
    emit_analysis_progress(
        progress_callback,
        "Controlli di significatività completati",
        68.0,
    )
    ...
    # STEP 10: Progress checkpoint - Indicatori temporali
    emit_analysis_progress(
        progress_callback,
        "Indicatori temporali completati",
        78.0,
    )
    ...
    if GENERATE_RESPONSE_MAP_LAYER:
        try:
            ...
            # STEP 11: Progress checkpoint - Response Map generata
            emit_analysis_progress(
                progress_callback,
                "Response Map generata",
                90.0,
            )
        except Exception as e:
    ...
    # STEP 12: Progress checkpoint - Validazione contratto dati
    emit_analysis_progress(
        progress_callback,
        "Validazione contratto dati",
        96.0,
    )
    ...
    # STEP 12: Progress checkpoint finale
    emit_analysis_progress(
        progress_callback,
        "Analisi completata",
        100.0,
...
# ================================================================
# ANALISI PRINCIPALE (SYNC - endpoint)
# ================================================================

@app.post("/analyze")
def analyze_field(
    payload: Dict[str, Any],
    auth_id: str = Depends(verify_api_key),
):
    """
    Endpoint sincrono per l'analisi satellitare.
    """
...
def create_analysis_job(
    ...
    geom_hash = compute_geojson_hash(geojson)
    job_id = build_job_id()
    ...
    jobs_executor.submit(run_analysis_job, job_id)
...
# ================================================================
# STEP 8: ENDPOINT GET /jobs/{job_id}
# ================================================================
...
def get_analysis_job(
    ...
    job = get_job_record_or_404(job_id, auth_id)

    return job_to_status_response(job)


# ================================================================
# STEP 9: ENDPOINT DELETE /jobs/{job_id}
# ================================================================
...
def cancel_analysis_job(
    ...
    with jobs_lock:
        ...
        if job["status"] in {"done", "error", "cancelled"}:
            response = job_to_status_response(deepcopy(job))
            return response
    ...
    audit_log(
        ...
        request_id=job_id,
        auth_id=auth_id,
        geometry_hash=job.get("geometry_hash"),
        analysis_profile=ANALYSIS_PROFILE,
    )
...
# ================================================================
# STEP 10: RUN ANALYSIS JOB (WORKER COMPLETO)
# ================================================================
...
def run_analysis_job(job_id: str) -> None:
    with jobs_lock:
        ...
        job.update({
            "status": "processing",
            "current_step": "Preparazione analisi",
            "progress_pct": 5.0,
            "started_at": utc_now_iso(),
        })

        auth_id = job["auth_id"]
        geojson = deepcopy(job["geojson"])
        geom_hash = job["geometry_hash"]
    ...
    try:
        ...
        update_job_record(
            job_id,
            current_step="Esecuzione analisi satellitare",
            progress_pct=15.0,
        )

        # STEP 13: Progress callback per il job
        def job_progress_callback(step: str, progress_pct: float) -> None:
            with jobs_lock:
                ...
                if current_job.get("status") not in {"queued", "processing"}:
                    return
        ...
        update_job_record(
            job_id,
            status="error",
            current_step="Errore interno durante l'analisi",
            progress_pct=None,
            result=None,
            error={
                "code": "analysis_job_failed",
                "message": "Errore durante l'elaborazione del job.",
                "error_id": error_id,
            },
            finished_at=utc_now_iso(),
        )
        ...
        print(f"[ERROR] analysis_job_failed job_id={job_id} error_id={error_id}")
        print(str(exc))
