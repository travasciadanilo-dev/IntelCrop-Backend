...
def fit_inner_search(
    ...
    search = GridSearchCV(
        estimator=build_pipeline(class_weight),
...
def main() -> None:
    ...
    balanced = aggregated_df[
        aggregated_df["model_name"]
        == "combined_balanced"
    ].copy()

    unweighted = aggregated_df[
        aggregated_df["model_name"]
        == "combined_unweighted"
    ].copy()
    ...
    folds_df.to_csv(
        output_dir
        / "regional_reliability_v4_weighting_folds.csv",
        index=False,
    )

    repeated_predictions_df.to_csv(
        output_dir
        / "regional_reliability_v4_weighting_repeated_predictions.csv",
        index=False,
    )

    aggregated_df.to_csv(
        output_dir
        / "regional_reliability_v4_weighting_aggregated_predictions.csv",
        index=False,
    )

    evaluation_df.to_csv(
        output_dir
        / "regional_reliability_v4_weighting_calibration_summary.csv",
        index=False,
    )

    lozo_df.to_csv(
        output_dir
        / "regional_reliability_v4_weighting_lozo.csv",
        index=False,
    )

    lozo_predictions_df.to_csv(
        output_dir
        / "regional_reliability_v4_weighting_lozo_predictions.csv",
        index=False,
    )

    bootstrap_df.to_csv(
        output_dir
        / "regional_reliability_v4_weighting_bootstrap.csv",
        index=False,
    )

    bootstrap_summary_df.to_csv(
        output_dir
        / "regional_reliability_v4_weighting_bootstrap_summary.csv",
        index=False,
    )

    print()
    print("Confronto calibrazione su OOF aggregate")
    print("--------------------------------------")
